...
void BroadcastOpHandle::RunImpl() {
  ...
  if (platform::is_cpu_place(in_tensor.place())) {
    ...
    for (auto out_var_handle : out_var_handles) {
      ...
      auto stream = static_cast<platform::CUDADeviceContext *>(dev_ctxes_[out_var_handle->place_])->stream();
      broadcast_calls.emplace_back(
          [send_recv_buffer, numel, type, root_id, &nccl_ctx, stream] {
            PADDLE_ENFORCE(platform::dynload::rcclBcast(
                send_recv_buffer, numel, static_cast<rcclDataType_t>(type),
                root_id, nccl_ctx.comm_, stream));
          });
#else
      broadcast_calls.emplace_back(
